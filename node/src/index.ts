...
const PROTOCOL = (process.env.PROXY_PROTOCOL || "vless") as
	| "vless"
	| "hysteria2";
...
const TTL_MINUTES = parseInt(
	process.env.TTL_MINUTES || `${15 + Math.floor(Math.random() * 46)}`,
); // random 15-60
...
async function main() {
	...
	if (PROXY_HOST !== "unknown") {
		config = buildProxyConfig(peerId);
		await announceProxyConfig(node, config);
	} else {
		console.log("⚠️  No PROXY_HOST set. Will announce when tunnel is ready.");
		config = buildProxyConfig(peerId); // build with 'unknown' host so we can re-announce later
	}

	// 3b. If host was unknown, poll for TUNNEL_HOST env var (up to 5 minutes)
	if (PROXY_HOST === "unknown" && config) {
		const maxPollMs = 5 * 60 * 1000;
		const pollIntervalMs = 10 * 1000;
		const pollStart = Date.now();
		const tunnelPoll = setInterval(async () => {
			const tunnelHost = process.env.TUNNEL_HOST;
			if (tunnelHost && tunnelHost !== "unknown") {
				clearInterval(tunnelPoll);
				try {
					const tunnelPort = process.env.TUNNEL_PORT
						? parseInt(process.env.TUNNEL_PORT)
						: undefined;
					await updateAndReannounce(node, config!, tunnelHost, tunnelPort);
				} catch (err) {
					console.error("Tunnel re-announce failed:", err);
				}
			} else if (Date.now() - pollStart >= maxPollMs) {
				clearInterval(tunnelPoll);
				console.log("⏰ Gave up waiting for TUNNEL_HOST after 5 minutes");
			}
		}, pollIntervalMs);
	}
	...
	const reannounceInterval = setInterval(
		async () => {
			if (config && config.host !== "unknown") {
				try {
					// Rebuild config to get fresh TTL
					const freshConfig = buildProxyConfig(peerId);
					await announceProxyConfig(node, freshConfig);
